---
title: |
    | STAT431 Project Report 
    | Bayesian Hierarchical Analysis of 
    | School Examination Results
author: 'Yang Du (yangd2), Anqi Li (anqili4), Hannah Pu (chpu2)'
date: "5/8/2019"
output: 
  pdf_document:
    toc: true
...
knitr::opts_chunk$set(echo = FALSE, out.width = '80%', out.height = '80%', fig.align = 'center')
# load packages
library(ggplot2)
library(gridExtra)
library(knitr)
...
data = read.table('Exam.txt', sep = ' ', col.names = c('SchoolID', 'StudentID', 'Score', 'Constant', 'LRT', 'Gender_Student', 'Gender_School', 'School_avg_score', 'Student_Verbal_score', 'Student_band_score' ))
cols <- c(1,2,6,7)
data[,cols] <- lapply(data[,cols], factor)
# load results
load('results_Hannah/x3_results100000.RData')
load('results_Hannah/x2_a1_results100000.RData')
load('results_Yang/mod2_a0a1/mod2_a0a1.RData')
load('results_Yang/mod2_bivariate/mod_2_bivariate.RData')
load('results_Anqi/results_x1/x1_results100000.RData')
load('results_Anqi/results_x2_u/x2_u_results100000.RData')
```
...
Figure \ref{fig:histbar} plots the histogram and bar plots of `Student gender` and `LRT` and the outcome variable : `Score`. It can be seen that the empirical distribution of LRT and score are centered at zero and have long and approximately equal left and right tail. Moreover, it's apparent that more girls are included in our data--2435 girls versus 1623 boys. Within most schools, more than 10 students were recorded and the number of recorded students are relatively balanced across most schools. Only two schools have recorded fewer than 10 students (School 48 and School 54). 

```{r, fig.cap='\\label{fig:histbar}Histogram and bar plots of single variables', fig.height=8, fig.pos='H'}
grid.arrange(
ggplot(data, aes(Score)) + 
  geom_histogram(bins  = 50),
ggplot(data, aes(LRT)) + 
  geom_histogram(bins  = 50),
ggplot(data, aes(Gender_Student)) +
  geom_bar(),
ggplot(data, aes(SchoolID)) +
  geom_bar() + coord_flip() + theme(axis.title.y = element_text(size = 6))
)
```

Figure \ref{fig:Scoreplots} shows the outcome variable `Score` against the other variables. It can be seen in the boxplot that school variations are manifest across schools while student gender doesn't seem to impact scores that much. The scatter plot between score and LRT shows an obvious positive relationship, higher LRT tends to have higher scores.

```{r, fig.cap='\\label{fig:Scoreplots}Plots of Score versus different variables', fig.height=8, fig.pos='H'}
grid.arrange(
ggplot(data, aes(SchoolID, Score)) +
  geom_boxplot(),
ggplot(data, aes(Gender_Student, Score)) +
  geom_boxplot(),
ggplot(data, aes(LRT, Score)) +
  geom_point()
)
```
...
```{r, fig.cap="\\label{fig:acyclic}Acyclic directed graphs", fig.height=10, fig.path='H'}
img1 = rasterGrob(as.raster(readPNG("Acyclic/mod1.png")), interpolate = FALSE)
img2 = rasterGrob(as.raster(readPNG("Acyclic/mod2A.png")), interpolate = FALSE)
img3 = rasterGrob(as.raster(readPNG("Acyclic/mod2B.png")), interpolate = FALSE)
img4 = rasterGrob(as.raster(readPNG("Acyclic/mod2C.png")), interpolate = FALSE)
img5 = rasterGrob(as.raster(readPNG("Acyclic/mod2D.png")), interpolate = FALSE)
img6 = rasterGrob(as.raster(readPNG("Acyclic/mod3.png")), interpolate = FALSE)
grid.arrange(img1, img2, img3, img4, img5, img6, ncol = 2, nrow = 3)
```

Acyclic directed graphs of all models are shown in figure \ref{fig:acyclic}. All JAGS code of the above mentioned models are shown in Appendix.

# 4. Analysis Results

## 4.1 Model Construction and Convergence check

&nbsp; &nbsp; &nbsp; &nbsp;  Using the models constructed in JAGS, hyperpriors were initialized and 3 chains were set for each model, the initialization values of the 3 chains were ranged from low values to high values (see appendix, Table \ref{tab:table1} to Table \ref{tab:table6}). Detailed R codes for running the models are shown in Appendix. Convergence is checked by looking at  traceplots, Gelman Rubin plots, and Gelman rubin statistics.The number of burin is determined by checking if all chains in traceplots overlapped and Gelmin Rubin statistic is below $1.05$. All models were first given 1000 initial runs and is noticed that convergence was not yet reached. After that, $100000$ more iterations were applied and all models successfully converged. Gelman Rubin statistics for all hyperparameters were around 1. Monte carlo error also shows to be less than 1/20 of the usual standard error (Table \ref{tab:table7}), which indicates that Monte carlo error is sufficiently low. Autocorrelation shows that there is not much autocorrelaton at higher lags. All traceplots, gelman rubin plots, and autocorrelation plots of the 100000 iterations are shown in appendix. 

```{r}
## se
load('model_yang_RData')
# mod1
mod1_SD = range(summary_x1$statistics[,2])
mod1_montese = range(summary_x1$statistics[,4])
mod1_se = cbind(mod1_SD[1], mod1_SD[2], mod1_montese[1], mod1_montese[2])
# mod2u
mod2_u_SD = range(summary_x2_u$statistics[,2])
mod2_u_montese = range(summary_x2_u$statistics[,4])
mod2_u_se = cbind(mod2_u_SD[1], mod2_u_SD[2], mod2_u_montese[1], mod2_u_montese[2])
# mod2b
mod2_b_SD = range(summary_x2_b$statistics[,2])
mod2_b_montese = range(summary_x2_b$statistics[,4])
mod2_b_se = cbind(mod2_b_SD[1], mod2_b_SD[2], mod2_b_montese[1], mod2_b_montese[2])
# mod2a1
mod2_a1_SD = range(summary_x2_a1$statistics[,2])
mod2_a1_montese = range(summary_x2_a1$statistics[,4])
mod2_a1_se = cbind(mod2_a1_SD[1], mod2_a1_SD[2], mod2_a1_montese[1], mod2_a1_montese[2])
# mod2a0a1
mod2_a0a1_SD = range(summary_x2_a0a1$statistics[,2])
mod2_a0a1_montese = range(summary_x2_a0a1$statistics[,4])
mod2_a0a1_se = cbind(mod2_a0a1_SD[1], mod2_a0a1_SD[2], mod2_a0a1_montese[1], mod2_a0a1_montese[2])
# mod2
mod3_SD = range(summary_x3$statistics[,2])
mod3_montese = range(summary_x3$statistics[,4])
mod3_se = cbind(mod3_SD[1], mod3_SD[2], mod3_montese[1], mod3_montese[2])
se_all = rbind(mod1_se, mod2_u_se, mod2_b_se, mod2_a1_se, mod2_a0a1_se, mod3_se)
se_all = cbind(rep(1,6), se_all)
se_all = round(se_all, 5)
colnames(se_all) = c("Gelman Rubin stats","SD (min)", "SD (max)", "Time-series SE (min)", "Time-series SE (max)")
rownames(se_all) = c("Model 1", "Model 2A", "Model 2B", "Model 2C", "Model 2D", "Model 3")
table7 = se_all
kable(table7, caption = "\\label{tab:table7}Monte Carlo errors and standard error in different models")
...
```{r}
dic <- cbind(10910, 9218, 9222, 9767, 9269, 9193)
colnames(dic) <- c('Model 1', 'Model 2A', 'Model 2B', 'Model 2C', 'Model 2D', 'Model 3')
rownames(dic) <- c('DIC')
knitr::kable(dic,caption='\\label{tab:dic}Deviance Information Criterion values of all models',format='pandoc',align=c('c','c','c','c','c','c'))
...
&nbsp; &nbsp; &nbsp; &nbsp;  The posterior correlation of $\beta_0$ and $\beta_1$ can be shown from the Figure \ref{fig:correlation}. It indicates that all $\beta_1$s are above 0, and $\beta_0$s are symmetric to 0. There is no obvious trend between $\beta_0$ and $\beta_1$. 

```{r, fig.cap="\\label{fig:correlation}Posterior correlation of beta0 and beta1", out.width='60%', out.height='60%',fig.pos='H'}
include_graphics("Rplot_x1.png")
```
